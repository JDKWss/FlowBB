...
2. **Attendance** to relacja `(User)-[:IS_GOING_TO]->(Event)` ze snapshotem `TransportMode`, `OriginLatitude`, `OriginLongitude`, `UpdatedAt`. Uzytkownik ma wewnetrzne, demonstracyjne `HomeLatitude` i `HomeLongitude`. Wspolrzedne nie sa czescia publicznego API.
