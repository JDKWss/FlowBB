...
| Stary graf (`Neo4jFlowBbGraphRepository*`, `Domain/Models/*`, `IFlowBbGraphRepository`) | uzywany przez initializer | usuniety w #16 | **usuniety** po przepisaniu initializera (3.B, zrobione) |
| Testy adapterow | `Neo4jPulseDataReaderTests`, `...RegistrationTests` (opt-in `NEO4J_RUN_INTEGRATION_TESTS`, zmienne `NEO4J_*`) | 41 testow (zmienne `FLOWBB_TEST_NEO4J_*`) | **jeden mechanizm**: wszystkie testy na `Neo4jFixture` i `Neo4jFactAttribute` (`FLOWBB_TEST_NEO4J_*`), testy Pulse z `develop` przeniesione; `Neo4jPulseSnapshotAdapterTests` z tej galezi zostaje |
...
- ZROBIONE: testy z `develop` (`Neo4jPulseDataReaderTests`) uzywaja `Neo4jFixture` i `FLOWBB_TEST_NEO4J_*` zamiast `NEO4J_*` (bazy aplikacji); usuniety `Neo4jIntegrationFactAttribute` i opt-in `NEO4J_RUN_INTEGRATION_TESTS`.
